...
def GCMS_augmentation(data, cat, computer_device):
    ...
    temperature_data = torch.tensor(temperature_data).unsqueeze(1).to(computer_device)
    composition_data = torch.tensor(combined_data / 100, dtype=torch.float32).to(computer_device)

    model = TemperatureToCompositionPredictor(input_size=1, output_size=10).to(computer_device)
    predicted_composition = GCMS_train_and_evaluate(model, temperature_data, composition_data)
    return predicted_composition

def FTIR_augmentation(FTIR_data, computer_device):
    ...
    # PyTorch 텐서로 변환
    temperatures = torch.tensor(temperature_data).unsqueeze(1).to(computer_device)  # (batch_size, 1, 1)
    outputs = torch.tensor(output_data).to(computer_device)

    # 모델 초기화
    model = TemperatureToDataPredictorCNN(input_size=1).to(computer_device)
    ...
    # 새로운 온도에서의 예측 및 시각화
    new_temperatures = torch.tensor([[275.0], [325.0], [375.0]], dtype=torch.float32).unsqueeze(1).to(computer_device)
    predict_and_plot(model, preprocessed_data, new_temperatures)
    return new_temperatures

def TGA_augmentation(TGA_data, cat, target_temp, computer_device, model_path='tga.pth', train_new_model=True):
...
if __name__ == '__main__' :
    ...
    if flag:
        condition_data, TGA_data, FTIR_data, GCMS_data = data_loader.load_data(data_loader.ROOT_DIR, data_type=None)
        ...
        if TGA_bool :
            augmented_TGA_data = TGA_augmentation(TGA_data, cat, target_temp, computer_device)
        ...
        new_temperatures = torch.tensor(new_temperatures).unsqueeze(1).to(computer_device)
        ...
        model = MVAE_MoE(input_dim1, input_dim2, latent_dim, output_dim, num_experts).to(computer_device)
