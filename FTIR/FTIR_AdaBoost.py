...
def FTIR_ABoost(only_predict = False):
    ...
    if only_predict :
        ...
        # AdaBoost 회귀 모델 정의
        base_estimator = DecisionTreeRegressor(max_depth=10)  # 기본 약한 학습기로 결정 트리 사용
        ada_model = AdaBoostRegressor(estimator=base_estimator, n_estimators=100, random_state=42)

        # 모델 학습
        ada_model.fit(X_scaled, y)

        # 학습된 모델과 스케일러 저장
        joblib.dump(ada_model, '/content/drive/MyDrive/lignin/pkl(AdaBoost)/ada_model.pkl')  # 모델 저장
        joblib.dump(scaler, '/content/drive/MyDrive/lignin/pkl(AdaBoost)/scaler.pkl')  # 스케일러 저장

        print("AdaBoost 회귀 모델과 스케일러를 저장했습니다.")


    # 저장된 AdaBoost 모델과 스케일러 불러오기
    ada_model = joblib.load('/content/drive/MyDrive/lignin/FT-IR/pkl(AdaBoost)/ada_model.pkl')  # AdaBoost 모델 파일 경로
    scaler = joblib.load('/content/drive/MyDrive/lignin/FT-IR/pkl(AdaBoost)/scaler.pkl')        # 스케일러 파일 경로
    ...
    # 예측 결과를 CSV 파일로 저장
    output_file_path = f'/content/drive/MyDrive/lignin/FT-IR/pkl(AdaBoost)/predict/{catalyst}{temperature}_predicted_ada.csv'
    predicted_data.to_csv(output_file_path, index=False)

    print(f"{temperature}℃에서 {catalyst}의 AdaBoost 예측 데이터를 {output_file_path}에 저장했습니다.")
